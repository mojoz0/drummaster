...
class Stick(object):
    def __init__(self, color):
        ...
        if self.color == "red":
            ...
            self.satRange = (100, 255)
            self.valRange = (70, 255)
        elif self.color == "blue":
            self.hueRange = (110, 140, 110, 140)
            self.satRange = (80, 255)
            self.valRange = (80, 255)
        self.rectPoints = (0, 0, 0, 0)
        self.last5Centers = [(0,0)]*5
    ...
    def findRectPoints(self, oldRectPoints):
        ...
        cv.InRangeS(hsv, (hueRange[0], satRange[0], valRange[0]),
                    (hueRange[1], satRange[1], satRange[1]),
                   threshold)
        cv.InRangeS(hsv, (hueRange[2], satRange[0], satRange[0]),
                    (hueRange[3], satRange[1], valRange[1]),
                    threshold2)
 ...
 #       cv.ShowImage(self.color, threshold)
    ...
    def findDelta(self):
        self.delta = abs(self.last5Centers[-1][0] - self.last5Centers[-3][0])

    def determineVolume(self):
        volumeConstant = float(self.delta)/50
        ...
        while True:
            if i %2 == 0:
                ...
                cv.PutText(frame, "Press q or esc to terminate.", (x, y),
                           font, (0, 255, 0))
                cv.PutText(frame, "Current Velocities:",
                           (x, y + 50), font, (0, 255, 255))
                cv.PutText(frame, str(self.redStick.delta), (x, y + 100),
                           font, (0, 0, 255))
                cv.PutText(frame, str(self.blueStick.delta), (x + 100, y + 100),
                          font, (255, 0, 0))
